...
(()=>{
  ...
  const normalizeUserCharacters=()=>{
    ...
    return read(CHAR_KEY).filter(character=>character?.gameId||(!character?.animeId&&!Array.isArray(character?.animeIds))).map(character=>{
...
(() => {
      ...
      function renderDashboard() {
        ...
        let characters=[];
        try{const saved=JSON.parse(localStorage.getItem('amorist-character-book-v1')||'[]');characters=Array.isArray(saved)?saved:[]}catch{}
      ...
      function dashboardCharacters(){
        try{
          ...
          const saved=Array.isArray(value)?value.filter(Boolean):[];
          const indexed=loadGames().flatMap(game=>(Array.isArray(game.sourceCharacters)?game.sourceCharacters:[]).map(character=>({...character,gameName:game.name,sourceGame:game.name})));
...
(() => {
      ...
      function readTimelineEvents(){try{const value=JSON.parse(localStorage.getItem(TIMELINE_KEY)||'[]');return Array.isArray(value)?value: value&&Array.isArray(value.events)?value.events:[]}catch{return[]}}
      ...
      function canonicalizeTimelineEvents(events){
        const output=[],routeIndexes=new Map();
        (Array.isArray(events)?events:[]).forEach(raw=>{
          const event=normalizeTimelineEvent(raw);
          if(event.type==='session'&&event.source==='migration'&&/^(?:通关路线|取消标记)\s*[：:]/.test(event.note))return;
          if(event.type==='route_completed'&&event.route){
            const key=`${event.gameId}\u0000${timelineRouteKey(event.route)}`;
            if(routeIndexes.has(key))output[routeIndexes.get(key)]=event;
            else{routeIndexes.set(key,output.length);output.push(event);}
            return;
          }
      ...
      function writeTimelineEvents(events){localStorage.setItem(TIMELINE_KEY,JSON.stringify({version:1,events:canonicalizeTimelineEvents(events)}));window.dispatchEvent(new CustomEvent('amorist-data-changed',{detail:{timeline:true}}));}
      function migrateTimelineData(){
        if(localStorage.getItem(TIMELINE_MIGRATION_KEY)==='1'){const current=readTimelineEvents(),canonical=canonicalizeTimelineEvents(current);if(JSON.stringify(current)!==JSON.stringify(canonical))writeTimelineEvents(canonical);return;}
        const events=readTimelineEvents(),existing=new Set(events.map(event=>String(event.id))),next=[...events];
        games().forEach(game=>{
          (Array.isArray(game.logs)?game.logs:[]).forEach((log,index)=>{const id=`legacy-log-${game.id}-${index}`;if(existing.has(id))return;const date=timelineDateValue(log.date);next.push(normalizeTimelineEvent({id,gameId:game.id,type:'session',occurredAt:date,datePrecision:timelinePrecision(date),title:'历史游玩记录',note:log.text,source:'migration'}));existing.add(id);});
          (Array.isArray(game.routeDone)?game.routeDone:[]).forEach(route=>{const id=`legacy-route-${game.id}-${String(route)}`;if(existing.has(id))return;const matching=(game.logs||[]).find(log=>String(log.text||'').includes(String(route))&&/通关|路线/.test(String(log.text||'')));const date=timelineDateValue(matching?.date);next.push(normalizeTimelineEvent({id,gameId:game.id,type:'route_completed',occurredAt:date,datePrecision:date?timelinePrecision(date):'unknown',route,note:matching?.text||'',source:'migration'}));existing.add(id);});
        });
        writeTimelineEvents(next);localStorage.setItem(TIMELINE_MIGRATION_KEY,'1');
      }
      ...
      function timelineTypeLabel(type){return ({started:'开始游玩',session:'游玩记录',progress:'进度更新',route_completed:'路线通关',completed:'游戏全通',paused:'暂停／封盘',rating:'评分记录',repo:'REPO',milestone:'纪念事件'})[type]||'历史记录';}
...
const routes=$('#libraryGameRoutes').value.split(/[，,]/).map(x=>x.trim()).filter(Boolean),progress=gameRouteProgress(routes,old.routeDone||[]);rows[index]={...old,category:dataModel.normalizeGameCategory($('#libraryGameCategory').value),platform:$('#libraryGamePlatform').value.trim(),hours:Number($('#libraryGameHours').value)||0,rating:Number($('#libraryGameRating').value)||0,routes,progress:progress==null?Number($('#libraryGameProgress').value)||0:progress,routeSelectionCustomized:true,routeDone:old.routeDone||[],logs:old.logs||[]};
      ...
      function renderGameDetail(id){
        ...
        const logRowsHtml=logs.length?logs.map((log,index)=>({log,index})).reverse().map(({log,index})=>window.AMORIST_MODE==='editor'?`<div class="game-log"><input class="game-log-date" type="date" value="${safe(gameLogDateValue(log.date))}" data-log-date="${index}" aria-label="修改日志日期"><span>${safe(log.text)}</span><button class="game-log-delete" type="button" data-log-delete="${index}" aria-label="删除这条日志">删除</button></div>`:`<div class="game-log game-log-public"><time>${safe(gameLogDateValue(log.date))}</time><span>${safe(log.text)}</span></div>`).join(''):'<span class="playing-meta">路线通关与手动记录会集中在这里。</span>';
        ...
        if(logButton){logButton.onclick=()=>window.openTimelineRecordDialog?.(id);const logHead=logButton.parentElement;logHead.classList.add('game-log-head');const backfillButton=document.createElement('button');backfillButton.className='product-button secondary small';backfillButton.type='button';backfillButton.textContent='历史补录';backfillButton.onclick=()=>window.openTimelineBackfill?.(id);logHead.append(backfillButton);}
        $$('#gameDetailPanel [data-route]').forEach(btn=>btn.onclick=()=>{
          ...
          const routeLogPattern=new RegExp(`^(?:通关路线|取消标记)\\s*[：:]\\s*${String(route).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}$`);
          const logs=(game.logs||[]).filter(log=>!routeLogPattern.test(String(log.text||'').trim()));
          if(!wasDone)logs.push({date:gameLogDateValue(),text:`通关路线：${route}`});
          const timelineEvents=readTimelineEvents().filter(event=>!isSameRouteEvent(event,id,route));
          if(!wasDone)timelineEvents.push(normalizeTimelineEvent({id:timelineRouteEventId(id,route),gameId:id,type:'route_completed',occurredAt:gameLogDateValue(),datePrecision:'day',route,title:'路线通关',source:'route-toggle'}));
          writeTimelineEvents(timelineEvents);
          saveGamePatch(id,{routeDone:next,progress:progress==null?game.progress:progress,logs});
        });
        $$('#gameDetailPanel [data-log-date]').forEach(input=>input.onchange=()=>{const index=Number(input.dataset.logDate),next=[...(game.logs||[])];if(!next[index])return;next[index]={...next[index],date:input.value||gameLogDateValue()};saveGamePatch(id,{logs:next})});
        $$('#gameDetailPanel [data-log-delete]').forEach(button=>button.onclick=()=>{const index=Number(button.dataset.logDelete),row=(game.logs||[])[index];if(!row||!confirm(`删除这条游玩日志？\n${row.text||''}`))return;saveGamePatch(id,{logs:(game.logs||[]).filter((_,i)=>i!==index)})});
...
(() => {
      ...
      const timelineFallbackRead=()=>{try{const value=JSON.parse(localStorage.getItem(TIMELINE_KEY_FALLBACK)||'[]');return Array.isArray(value)?value:Array.isArray(value?.events)?value.events:[]}catch{return[]}};
      const timelineFallbackWrite=events=>{localStorage.setItem(TIMELINE_KEY_FALLBACK,JSON.stringify({version:1,events}));window.dispatchEvent(new CustomEvent('amorist-data-changed',{detail:{timeline:true}}))};
      ...
      function renderCharacterBook(query=''){
        const all=chars(),gameRows=games(),gameMap=Object.fromEntries(gameRows.map(g=>[g.id,g]));
      ...
      async function createCharacterFromBangumi(c){
        ...
        const preferredSubjectId=String(bgSelected?.id||'')&&subjectIds.includes(String(bgSelected.id))?String(bgSelected.id):subjectIds.find(subjectId=>games().some(game=>String(game.bangumiId||'')===subjectId))||existing?.bangumiSubjectId||subjectIds[0]||'';
        const linkedGame=games().find(game=>String(game.bangumiId||'')===preferredSubjectId)||games().find(game=>subjectIds.includes(String(game.bangumiId||'')));
        const record=dataModel.normalizeCharacterRecord({...(existing||{}),id:existing?.id||`char-bgm-${c.id}`,name:c.name||'未命名角色',nameCn:c.name_cn||existing?.nameCn||'',aliases:existing?.aliases||[],gameId:existing?.gameId||linkedGame?.id||'',gameIds:[...new Set([...(Array.isArray(existing?.gameIds)?existing.gameIds:[]),existing?.gameId,linkedGame?.id].filter(Boolean))],bangumiCharacterId:c.id,bangumiSubjectId:preferredSubjectId,bangumiSubjectIds:subjectIds,preference:dataModel.normalizeCharacterPreference(existing?.preference??existing?.relation),roleType:existing?.roleType||'unset',roleTypeSource:existing?.roleTypeSource||'bangumi',cv:existing?.cv||cv,image:existing?.image||image,note:existing?.note||'',summary:existing?.summary||'',infobox:existing?.infobox||[],nameSearchSynced:Boolean(c.name_cn||existing?.nameSearchSynced),updatedAt:Date.now()},games());
      ...
      function renderTimeline(){
        const gameRows=games(),events=[];
        gameRows.forEach(game=>{
          if(game.updatedAt)events.push({date:game.updatedAt,gameId:game.id,gameName:game.name,cover:game.cover||'',type:'update',text:`更新了「${game.name}」的档案`,status:game.status});
          if(Array.isArray(game.logs)){game.logs.forEach(log=>{events.push({date:log.date,gameId:game.id,gameName:game.name,cover:game.cover||'',type:'log',text:log.text})});}
          if(Array.isArray(game.routeDone)){game.routeDone.forEach(route=>{events.push({date:game.updatedAt||Date.now(),gameId:game.id,gameName:game.name,cover:game.cover||'',type:'route',text:`通关路线：${route}`})});}
        });
        events.sort((a,b)=>{
        ...
        const host=$('#timelineContent');
        if(!events.length){host.innerHTML='<div class="timeline-empty"><strong>时间线还是空的</strong>开始玩游戏、标记路线或写 REPO 后，这里会自动汇聚成你的编年史。</div>';return;}
        const months={};
        events.forEach(ev=>{
          const d=typeof ev.date==='number'?new Date(ev.date):new Date(ev.date);
          if(isNaN(d.getTime()))return;
          const key=`${d.getFullYear()}.${String(d.getMonth()+1).padStart(2,'0')}`;
          if(!months[key])months[key]=[];
          months[key].push({...ev,day:d.getDate(),month:d.getMonth()+1});
        });
        host.innerHTML=Object.entries(months).map(([key,evs])=>{
          const count=evs.length;
          return `<div class="timeline-month"><div class="timeline-month-head"><h3>${key}</h3><span>${count} 条记录</span></div>${evs.map(ev=>{
            const badgeClass=ev.type==='route'?'route':ev.type==='log'?'':'';
            const badgeText=ev.type==='route'?'路线通关':ev.type==='log'?'游玩记录':ev.status||'更新';
            return `<div class="timeline-entry"><div class="timeline-date">${ev.day}日</div><div class="timeline-body"><div class="timeline-cover">${ev.cover?`<img src="${safe(ev.cover)}" alt="${safe(ev.gameName)}" referrerpolicy="no-referrer">`:safe(initial(ev.gameName))}</div><div class="timeline-text"><strong>${safe(ev.gameName)}</strong><p>${safe(ev.text)}</p><span class="timeline-badge ${badgeClass}">${safe(badgeText)}</span></div></div></div>`;
          }).join('')}</div>`;
        }).join('');
      }
      window.renderTimeline=renderTimeline;
      window.openTimelineRecordDialog=openTimelineRecordDialog;
      $('#timelineBackfillForm')?.addEventListener('submit',()=>{const gameId=$('#timelineBackfillGameId').value;$$('#timelineBackfillRoutes .backfill-route-row').forEach(row=>{const route=row.dataset.backfillRoute,done=row.querySelector('.backfill-route-done')?.checked,precision=row.querySelector('.backfill-route-precision')?.value||'unknown',date=eventDateForPrecision(row.querySelector('.backfill-route-date')?.value||'',precision),note=row.querySelector('.backfill-route-note')?.value.trim()||'';replaceBackfillEvent(gameId,'route_completed',done?{occurredAt:date,datePrecision:precision,route,note,title:'路线通关'}:null,route)})},{capture:true});
      $('#timelineBackfillForm')?.addEventListener('submit',()=>{const gameId=$('#timelineBackfillGameId').value,raw=timelineFallbackRead(),precisionFor=key=>$(`#timelineBackfill${key}Precision`).value||'unknown',dateFor=key=>eventDateForPrecision($(`#timelineBackfill${key}Date`).value,precisionFor(key)),upsert=(type,occurredAt,key)=>{const id=backfillEventId(gameId,type),next=raw.filter(event=>event.id!==id);if(occurredAt||precisionFor(key)==='unknown')next.push({id,gameId,type,occurredAt,datePrecision:precisionFor(key),title:timelineTypeLabel(type),source:'backfill'});return next};let next=upsert('started',dateFor('Start'),'Start');next=upsert('completed',dateFor('Complete'),'Complete');next=upsert('paused',dateFor('Paused'),'Paused');timelineFallbackWrite(next)},{capture:true});
      const BACKFILL_DRAFT_KEY='amorist-timeline-backfill-drafts-v1';
      const readBackfillDrafts=()=>{try{return JSON.parse(localStorage.getItem(BACKFILL_DRAFT_KEY)||'{}')}catch{return{}}};
      const restoreBackfillDraft=()=>{const id=$('#timelineBackfillGameId')?.value;if(!id)return;const draft=readBackfillDrafts()[id];if(!draft)return;[['Start','start'],['Complete','complete'],['Paused','paused']].forEach(([key,name])=>{$(`#timelineBackfill${key}Date`).value=draft[`${name}Date`]||'';$(`#timelineBackfill${key}Precision`).value=draft[`${name}Precision`]||'unknown'});$('#timelineBackfillNote').value=draft.note||''};
      $('#timelineBackfillForm')?.addEventListener('submit',()=>{const id=$('#timelineBackfillGameId').value,drafts=readBackfillDrafts();drafts[id]={startDate:$('#timelineBackfillStartDate').value,startPrecision:$('#timelineBackfillStartPrecision').value,completeDate:$('#timelineBackfillCompleteDate').value,completePrecision:$('#timelineBackfillCompletePrecision').value,pausedDate:$('#timelineBackfillPausedDate').value,pausedPrecision:$('#timelineBackfillPausedPrecision').value,note:$('#timelineBackfillNote').value};localStorage.setItem(BACKFILL_DRAFT_KEY,JSON.stringify(drafts))},{capture:true});
      ['timelineBackfillPrev','timelineBackfillNext'].forEach(id=>document.getElementById(id)?.addEventListener('click',()=>setTimeout(restoreBackfillDraft,0)));
      ['timelineBackfillStartDate','timelineBackfillCompleteDate','timelineBackfillPausedDate'].forEach(id=>document.getElementById(id)?.addEventListener('input',event=>{if(event.target.value){const key=id.replace('timelineBackfill','').replace('Date','');$(`#timelineBackfill${key}Precision`).value='day'}}));
      window.openTimelineBackfill=openTimelineBackfill;

      /* Structured timeline UI */
      timelineStore?.migrate?.();
      let timelineState={type:'all',game:'all',year:'all',unknown:true},backfillIds=[],backfillIndex=0;
      const localDate=()=>{const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`};
      const eventDateForPrecision=(value,precision)=>{const raw=timelineDateValue(value);if(!raw)return '';if(precision==='year')return raw.slice(0,4);if(precision==='month')return raw.slice(0,7);return raw};
      function timelineUpsert(event){const replaceId=event?._replaceId,row=normalizeTimelineEvent({...event,updatedAt:Date.now()});delete row._replaceId;const next=readTimelineEvents().map(normalizeTimelineEvent).filter(item=>item.id!==row.id&&item.id!==replaceId&&!(row.type==='route_completed'&&row.route&&sameTimelineRoute(item,row.gameId,row.route)));next.push(row);writeTimelineEvents(next)}
      function routeOptions(game,selected=''){return ['<option value="">不关联路线</option>',...gameRouteNames(game).map(route=>`<option value="${safe(route)}" ${route===selected?'selected':''}>${safe(route)}</option>`)].join('')}
      function openTimelineRecordDialog(gameId,event=null){const game=games().find(row=>String(row.id)===String(gameId));if(!game)return;const overlay=$('#timelineRecordOverlay');if(!overlay)return;$('#timelineRecordGameId').value=game.id;$('#timelineRecordEventId').value=event?.id||'';$('#timelineRecordTitle').textContent=event?'编辑时间线记录':`新增记录 · ${game.name}`;$('#timelineRecordType').value=event?.type||'session';$('#timelineRecordDate').value=event?.occurredAt||localDate();$('#timelineRecordPrecision').value=event?.datePrecision||'day';$('#timelineRecordRoute').innerHTML=routeOptions(game,event?.route||'');$('#timelineRecordProgress').value=event?.progress??game.progress??'';$('#timelineRecordHours').value=event?.playHours??'';$('#timelineRecordTitleInput').value=event?.title||'';$('#timelineRecordNote').value=event?.note||'';$('#timelineRecordRouteGroup').hidden=!['route_completed','progress'].includes($('#timelineRecordType').value);overlay.classList.add('open')}
      function closeTimelineRecordDialog(){$('#timelineRecordOverlay')?.classList.remove('open')}
      function saveTimelineRecord(event){const game=games().find(row=>String(row.id)===String(event.gameId));if(!game)return;timelineUpsert({...event,title:event.title||timelineTypeLabel(event.type),source:event.source||'manual'});if(event.type==='route_completed'&&event.route){const done=[...new Set([...(game.routeDone||[]),event.route])],progress=gameRouteProgress(gameRouteNames(game),done);saveGames(games().map(row=>row.id===game.id?{...row,routeDone:done,progress:progress==null?row.progress:progress}:row))}else if(event.type==='completed'){saveGames(games().map(row=>row.id===game.id?{...row,progress:100}:row))}else if(event.progress!=null&&!Number.isNaN(Number(event.progress))){saveGames(games().map(row=>row.id===game.id?{...row,progress:Number(event.progress)}:row))}}
      $('#timelineRecordType')?.addEventListener('change',()=>{$('#timelineRecordRouteGroup').hidden=!['route_completed','progress'].includes($('#timelineRecordType').value)});
      $('#timelineRecordForm')?.addEventListener('submit',event=>{event.preventDefault();const type=$('#timelineRecordType').value,precision=$('#timelineRecordPrecision').value,gameId=$('#timelineRecordGameId').value,existingId=$('#timelineRecordEventId').value,route=$('#timelineRecordRoute').value,row={id:type==='route_completed'&&route?timelineRouteId(gameId,route):(existingId||timelineId('event')),_replaceId:existingId,gameId,type,occurredAt:eventDateForPrecision($('#timelineRecordDate').value,precision),datePrecision:precision,route,title:$('#timelineRecordTitleInput').value.trim(),note:$('#timelineRecordNote').value.trim(),progress:$('#timelineRecordProgress').value===''?null:Number($('#timelineRecordProgress').value),playHours:$('#timelineRecordHours').value===''?null:Number($('#timelineRecordHours').value),source:'manual'};saveTimelineRecord(row);closeTimelineRecordDialog();renderTimeline();if($('#gameDetailPanel')&&!$('#gameDetailPanel').hidden)renderGameDetail(gameId);toast('时间线记录已保存')});
      $('#cancelTimelineRecord')?.addEventListener('click',closeTimelineRecordDialog);$('#closeTimelineRecord')?.addEventListener('click',closeTimelineRecordDialog);
      function backfillEventId(gameId,type,route=''){return ['route','route_completed'].includes(type)&&route?timelineRouteId(gameId,route):`backfill-${gameId}-${type}-${route?encodeURIComponent(route):'main'}`}
      function renderBackfillRoutes(game){const host=$('#timelineBackfillRoutes'),events=readTimelineEvents(),routes=gameRouteNames(game);host.innerHTML=routes.map(route=>{const ev=events.find(item=>sameTimelineRoute(item,game.id,route));return `<div class="backfill-route-row" data-backfill-route="${safe(route)}"><strong>${safe(route)}</strong><label><input type="checkbox" class="backfill-route-done" ${ev?'checked':''}>已通关</label><input class="product-input backfill-route-date" placeholder="日期 / 年月" value="${safe(ev?.occurredAt||'')}"><select class="product-select backfill-route-precision"><option value="day" ${ev?.datePrecision==='day'?'selected':''}>日</option><option value="month" ${ev?.datePrecision==='month'?'selected':''}>月</option><option value="year" ${ev?.datePrecision==='year'?'selected':''}>年</option><option value="unknown" ${!ev||ev?.datePrecision==='unknown'?'selected':''}>不确定</option></select><input class="product-input backfill-route-note" placeholder="备注" value="${safe(ev?.note||'')}"></div>`}).join('')||'<p class="playing-meta">暂无路线，可添加自定义路线。</p>'}
      function openTimelineBackfill(gameId){backfillIds=games().map(game=>game.id);backfillIndex=Math.max(0,backfillIds.findIndex(id=>String(id)===String(gameId)));loadTimelineBackfill(backfillIds[backfillIndex])}
      function loadTimelineBackfill(gameId){const game=games().find(row=>String(row.id)===String(gameId));if(!game)return;$('#timelineBackfillGameId').value=game.id;$('#timelineBackfillGameName').textContent=game.name;$('#timelineBackfillProgress').textContent=`${backfillIndex+1} / ${backfillIds.length}`;[['Start','started'],['Complete','completed'],['Paused','paused']].forEach(([key,type])=>{const ev=readTimelineEvents().find(item=>item.id===backfillEventId(game.id,type));$(`#timelineBackfill${key}Date`).value=ev?.occurredAt||'';$(`#timelineBackfill${key}Precision`).value=ev?.datePrecision||'unknown'});const rating=readTimelineEvents().find(ev=>ev.id===backfillEventId(game.id,'rating'));$('#timelineBackfillRating').value=rating?.rating??game.rating??'';$('#timelineBackfillHours').value=rating?.playHours??game.hours??'';const note=readTimelineEvents().find(ev=>ev.id===backfillEventId(game.id,'milestone'));$('#timelineBackfillNote').value=note?.note||'';renderBackfillRoutes(game);$('#timelineBackfillOverlay').classList.add('open')}
      function replaceBackfillEvent(gameId,type,data,route=''){const canonicalType=type==='route'?'route_completed':type,id=backfillEventId(gameId,canonicalType,route),events=readTimelineEvents().filter(event=>event.id!==id&&!(canonicalType==='route_completed'&&route&&sameTimelineRoute(event,gameId,route)));if(data)events.push(normalizeTimelineEvent({...data,id,gameId,type:canonicalType,route:data.route||route,source:'backfill'}));writeTimelineEvents(events)}
      function saveBackfill(andNext=true){const gameId=$('#timelineBackfillGameId').value,game=games().find(row=>String(row.id)===String(gameId));if(!game)return;[['Start','started'],['Complete','completed'],['Paused','paused']].forEach(([key,type])=>{const precision=$(`#timelineBackfill${key}Precision`).value,date=eventDateForPrecision($(`#timelineBackfill${key}Date`).value,precision);replaceBackfillEvent(game.id,type,date||precision==='unknown'?{occurredAt:date,datePrecision:precision,title:timelineTypeLabel(type)}:null)});const rating=$('#timelineBackfillRating').value,hours=$('#timelineBackfillHours').value,note=$('#timelineBackfillNote').value.trim();replaceBackfillEvent(game.id,'rating',rating||hours?{occurredAt:'',datePrecision:'unknown',title:'评分与时长',rating:rating?Number(rating):null,playHours:hours?Number(hours):null}:null);replaceBackfillEvent(game.id,'milestone',note?{occurredAt:'',datePrecision:'unknown',title:'历史补录备注',note}:null);const rows=games(),done=[...document.querySelectorAll('#timelineBackfillRoutes .backfill-route-row')].filter(row=>row.querySelector('.backfill-route-done')?.checked).map(row=>row.dataset.backfillRoute);rows.forEach(row=>{if(row.id===game.id){row.routeDone=done;row.rating=rating?Number(rating):row.rating;row.hours=hours?Number(hours):row.hours;const progress=gameRouteProgress(gameRouteNames(row),done);if(progress!=null)row.progress=progress}});saveGames(rows);if(andNext&&backfillIndex<backfillIds.length-1){backfillIndex++;loadTimelineBackfill(backfillIds[backfillIndex])}else{$('#timelineBackfillOverlay').classList.remove('open');renderTimeline()}toast('历史补录已保存')}
      $('#timelineBackfillForm')?.addEventListener('submit',event=>{event.preventDefault();saveBackfill(true)});$('#cancelTimelineBackfill')?.addEventListener('click',()=>$('#timelineBackfillOverlay').classList.remove('open'));$('#closeTimelineBackfill')?.addEventListener('click',()=>$('#timelineBackfillOverlay').classList.remove('open'));$('#timelineBackfillPrev')?.addEventListener('click',()=>{saveBackfill(false);if(backfillIndex>0){backfillIndex--;loadTimelineBackfill(backfillIds[backfillIndex])}});$('#timelineBackfillNext')?.addEventListener('click',()=>{saveBackfill(false);if(backfillIndex<backfillIds.length-1){backfillIndex++;loadTimelineBackfill(backfillIds[backfillIndex])}});$('#timelineBackfillAddRoute')?.addEventListener('click',()=>{const name=prompt('请输入路线名称');if(!name?.trim())return;const id=$('#timelineBackfillGameId').value,rows=games(),game=rows.find(row=>String(row.id)===String(id));if(game){game.routes=[...new Set([...gameRouteNames(game),name.trim()])];game.routeSelectionCustomized=true;saveGames(rows);renderBackfillRoutes(game)}});$('#timelineRoutesAllDone')?.addEventListener('click',()=>$$('#timelineBackfillRoutes .backfill-route-done').forEach(input=>input.checked=true));$('#timelineRoutesClearDone')?.addEventListener('click',()=>$$('#timelineBackfillRoutes .backfill-route-done').forEach(input=>input.checked=false));$('#timelineRoutesClearDates')?.addEventListener('click',()=>$$('#timelineBackfillRoutes .backfill-route-date').forEach(input=>input.value=''));$('#timelineRoutesCopyDate')?.addEventListener('click',()=>{const dates=$$('#timelineBackfillRoutes .backfill-route-date'),value=dates.find(input=>input.value)?.value||'';dates.forEach(input=>{if(!input.value)input.value=value})});
      function timelineControlHtml(gameRows,events){const years=[...new Set(events.map(e=>String(e.occurredAt||'').slice(0,4)).filter(Boolean))].sort().reverse();return `<div class="timeline-toolbar"><select class="product-select" id="timelineTypeFilter"><option value="all">全部记录</option><option value="session">游玩记录</option><option value="route_completed">路线通关</option><option value="completed">游戏全通</option><option value="repo">REPO</option><option value="milestone">里程碑</option></select><select class="product-select" id="timelineGameFilter"><option value="all">所有游戏</option>${gameRows.map(g=>`<option value="${safe(g.id)}">${safe(g.name)}</option>`).join('')}</select><select class="product-select" id="timelineYearFilter"><option value="all">全部年份</option>${years.map(year=>`<option>${year}</option>`).join('')}</select><label class="timeline-check"><input type="checkbox" id="timelineUnknownFilter" ${timelineState.unknown?'checked':''}>包含日期不确定</label>${window.AMORIST_MODE==='editor'?'<button class="product-button secondary small" id="timelineBackfillOpen" type="button">历史补录</button>':''}</div>`}
      function renderTimeline(){const host=$('#timelineContent');if(!host)return;const gameRows=games(),all=readTimelineEvents().map(normalizeTimelineEvent),filtered=all.filter(event=>(timelineState.type==='all'||event.type===timelineState.type)&&(timelineState.game==='all'||String(event.gameId)===String(timelineState.game))&&(timelineState.year==='all'||String(event.occurredAt).startsWith(timelineState.year))&&(timelineState.unknown||event.datePrecision!=='unknown'));const groups={};filtered.sort((a,b)=>eventSortValue(b)-eventSortValue(a)).forEach(event=>{const key=event.datePrecision==='unknown'?'日期待确认':String(event.occurredAt||'').slice(0,7)||'日期待确认';(groups[key]??=[]).push(event)});const body=Object.entries(groups).map(([key,events])=>`<div class="timeline-month"><div class="timeline-month-head"><h3>${safe(key)}</h3><span>${events.length} 条记录</span></div>${events.map(event=>{const game=gameRows.find(row=>String(row.id)===String(event.gameId)),name=game?.name||timelineGameName(event.gameId),cover=game?.cover||'';return `<article class="timeline-entry timeline-event-card" data-timeline-event="${safe(event.id)}" data-game-id="${safe(event.gameId)}"><div class="timeline-date">${safe(timelineDateLabel(event))}</div><div class="timeline-body"><div class="timeline-cover">${cover?`<img src="${safe(cover)}" alt="${safe(name)}" referrerpolicy="no-referrer">`:safe(initial(name))}</div><div class="timeline-text"><strong>${safe(name)}</strong><span class="timeline-badge ${event.type==='route_completed'?'route':''}">${safe(timelineTypeLabel(event.type))}${event.route?` · ${safe(event.route)}`:''}</span>${event.title?`<h4>${safe(event.title)}</h4>`:''}<p>${safe(event.note||'')}</p>${window.AMORIST_MODE==='editor'?`<div class="timeline-event-actions"><button type="button" data-timeline-edit="${safe(event.id)}">编辑</button><button type="button" data-timeline-delete="${safe(event.id)}">删除</button></div>`:''}</div></div></article>`}).join('')}</div>`).join('');host.innerHTML=timelineControlHtml(gameRows,all)+(body||'<div class="timeline-empty"><strong>还没有时间线记录</strong><p>从游戏档案新增记录，或使用历史补录一次整理过去的游玩经历。</p></div>');$('#timelineTypeFilter').value=timelineState.type;$('#timelineTypeFilter').onchange=e=>{timelineState.type=e.target.value;renderTimeline()};$('#timelineGameFilter').value=timelineState.game;$('#timelineGameFilter').onchange=e=>{timelineState.game=e.target.value;renderTimeline()};$('#timelineYearFilter').value=timelineState.year;$('#timelineYearFilter').onchange=e=>{timelineState.year=e.target.value;renderTimeline()};$('#timelineUnknownFilter').onchange=e=>{timelineState.unknown=e.target.checked;renderTimeline()};const backfillButton=$('#timelineBackfillOpen');if(backfillButton)backfillButton.onclick=()=>openTimelineBackfill(gameRows[0]?.id);host.querySelectorAll('[data-timeline-edit]').forEach(button=>button.onclick=e=>{e.stopPropagation();const event=all.find(row=>row.id===button.dataset.timelineEdit);if(event)openTimelineRecordDialog(event.gameId,event)});host.querySelectorAll('[data-timeline-delete]').forEach(button=>button.onclick=e=>{e.stopPropagation();if(!confirm('删除这条时间线记录？'))return;writeTimelineEvents(readTimelineEvents().filter(row=>row.id!==button.dataset.timelineDelete));renderTimeline()});host.querySelectorAll('.timeline-event-card').forEach(card=>card.onclick=e=>{if(e.target.closest('button'))return;const id=card.dataset.gameId;go('library');setTimeout(()=>renderGameDetail(id),0)})}
      ...
      $('#assetPickerList').addEventListener('click',e=>{
        ...
        if(charPick){
          const char=chars().find(c=>c.id===charPick.dataset.assetChar),st=newVisualState();
      ...
      window.openVisualTemplate=function(id,seedGameId='',seedCharId=''){
        if(newVisualTemplates[id]){
          ...
          $('#formTemplates').style.display='none';$('#formWorkspace').classList.add('active');$('#backToTemplates').hidden=false;
          if(seedCharId){const st=newVisualState(),char=chars().find(c=>c.id===seedCharId);if(char&&!st.slots[0]){st.slots[0]={charId:char.id,name:char.name,cover:char.image||'',label:char.name};newSaveVisual(st);}}
      ...
      window.addEventListener('amorist-data-changed',event=>{
        if(!event.detail||event.detail.chars)renderCharacterBook($('#charSearchInput')?.value||'');
...
/* ══════════════════════════════════════════════════════════
  ...
  ;(function(){
    ...
    const characterHistoryNameKey=value=>String(value||'').normalize('NFKC').toLowerCase().replace(/[\s・·\-—_「」『』【】（）()]/g,'');
    function characterHistoryHtml(remote,shared,linked,subjectIds,localGame){
      const timelineStore=window.AmoristTimelineStore;
      const gameRows=games();
      const gameIds=new Set([
        shared?.gameId,
        ...(Array.isArray(shared?.gameIds)?shared.gameIds:[]),
        localGame?.id,
        ...gameRows.filter(game=>subjectIds.includes(String(game.bangumiId||game.bangumiDisplayId||''))).map(game=>game.id)
      ].filter(Boolean).map(String));
      if(!gameIds.size)return '';
      const nameKeys=new Set([
        remote?.name,remote?.name_cn,linked?.name,linked?.nameCn,shared?.name,shared?.nameCn,
        ...(Array.isArray(shared?.aliases)?shared.aliases:[]),
        ...(Array.isArray(linked?.aliases)?linked.aliases:[])
      ].map(characterHistoryNameKey).filter(Boolean));
      const routeMatches=value=>{const routeKey=characterHistoryNameKey(value);return routeKey&&[...nameKeys].some(nameKey=>routeKey===nameKey||(nameKey.length>1&&routeKey.includes(nameKey)));};
      const rows=(timelineStore?.read?.()||[]).map(event=>timelineStore?.normalize?.(event)||event).filter(event=>{
        if(!gameIds.has(String(event?.gameId||'')))return false;
        return event.type!=='route_completed'||routeMatches(event.route);
      }).sort((a,b)=>(timelineStore?.sort?.(b)||0)-(timelineStore?.sort?.(a)||0));
      const content=rows.map(event=>{
        const game=gameRows.find(item=>String(item.id)===String(event.gameId)),date=timelineStore?.dateLabel?.(event)||event.occurredAt||'日期待确认';
        const type=timelineStore?.typeLabel?.(event.type)||event.type||'游玩记录';
        const detail=[event.route?`角色路线：${event.route}`:'',event.title,event.note].filter(Boolean).join(' · ');
        return `<article class="character-history-entry"><time>${safe(date)}</time><div><strong>${safe(type)}${game?.name?` · ${safe(game.name)}`:''}</strong>${detail?`<p>${safe(detail)}</p>`:''}</div></article>`;
      }).join('');
      return `<section class="bangumi-char-section character-history-section"><h3>游玩日志</h3><div class="character-history-list">${content||'<p class="character-history-empty">还没有与这位角色关联的游玩记录。</p>'}</div></section>`;
    }
    async function showCharacterDetail(id){
      ...
      try{
        ...
        const history=characterHistoryHtml(s,shared,linked,subjectIds,localGame);
...
(()=>{
  ...
  const read=(key,fallback)=>{try{const v=JSON.parse(localStorage.getItem(key)||'');return v??fallback}catch{return fallback}};
      const games=()=>read(GAME_KEY,[]).filter(Boolean), chars=()=>read(CHAR_KEY,[]).filter(character=>character?.gameId||(!character?.animeId&&!Array.isArray(character?.animeIds))).filter(Boolean);
