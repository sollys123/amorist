...
(() => {
      ...
      function renderTimeline(){
        const host=$('#timelineContent');if(!host)return;const gameRows=games(),all=readTimelineEvents().map(normalizeTimelineEvent).filter(event=>['started','completed'].includes(event.type));
        ...
        host.innerHTML=timelineControlHtml(gameRows,all)+(timelineState.view==='calendar'?timelineCalendarHtml(gameRows,filtered):(body||'<div class="timeline-empty"><strong>还没有游玩时间</strong><p>时间线只记录游戏的开始游玩时间和全通时间。</p></div>'));
