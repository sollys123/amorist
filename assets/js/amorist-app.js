...
(() => {
      ...
      const TIMELINE_ALLOWED_TYPES=new Set(['started','completed']);
      ...
      function timelineTypeLabel(type){return ({started:'开始游玩',completed:'游戏全通'})[type]||'游玩时间';}
...
(() => {
      ...
      const timelineFallbackRead=()=>{try{const value=JSON.parse(localStorage.getItem(TIMELINE_KEY_FALLBACK)||'[]'),events=Array.isArray(value)?value:Array.isArray(value?.events)?value.events:[];return events.filter(event=>['started','completed'].includes(String(event?.type||'')))}catch{return[]}};
      const timelineFallbackWrite=events=>{localStorage.setItem(TIMELINE_KEY_FALLBACK,JSON.stringify({version:2,events:(Array.isArray(events)?events:[]).filter(event=>['started','completed'].includes(String(event?.type||'')))}));window.dispatchEvent(new CustomEvent('amorist-data-changed',{detail:{timeline:true}}))};
      ...
      const restoreBackfillDraft=()=>{const id=$('#timelineBackfillGameId')?.value;if(!id)return;const draft=readBackfillDrafts()[id];if(!draft)return;[['Start','start'],['Complete','complete']].forEach(([key,name])=>{$(`#timelineBackfill${key}Date`).value=draft[`${name}Date`]||'';$(`#timelineBackfill${key}Precision`).value=draft[`${name}Precision`]||'unknown'})};
      $('#timelineBackfillForm')?.addEventListener('submit',()=>{const id=$('#timelineBackfillGameId').value,drafts=readBackfillDrafts();drafts[id]={startDate:$('#timelineBackfillStartDate').value,startPrecision:$('#timelineBackfillStartPrecision').value,completeDate:$('#timelineBackfillCompleteDate').value,completePrecision:$('#timelineBackfillCompletePrecision').value};localStorage.setItem(BACKFILL_DRAFT_KEY,JSON.stringify(drafts))},{capture:true});
      ...
      ['timelineBackfillStartDate','timelineBackfillCompleteDate'].forEach(id=>document.getElementById(id)?.addEventListener('input',event=>{if(event.target.value){const key=id.replace('timelineBackfill','').replace('Date','');$(`#timelineBackfill${key}Precision`).value='day'}}));
      ...
      function timelineUpsert(event){if(!['started','completed'].includes(String(event?.type||'')))return;const replaceId=event?._replaceId,row=normalizeTimelineEvent({...event,updatedAt:Date.now()});delete row._replaceId;row.id=replaceId||backfillEventId(row.gameId,row.type);const next=readTimelineEvents().map(normalizeTimelineEvent).filter(item=>item.id!==row.id&&item.id!==replaceId&&!(item.gameId===row.gameId&&item.type===row.type));next.push(row);writeTimelineEvents(next)}
      function openTimelineRecordDialog(gameId,event=null){const game=games().find(row=>String(row.id)===String(gameId));if(!game)return;const overlay=$('#timelineRecordOverlay');if(!overlay)return;const type=['started','completed'].includes(event?.type)?event.type:'started';$('#timelineRecordGameId').value=game.id;$('#timelineRecordEventId').value=event?.id||'';$('#timelineRecordTitle').textContent=event?'编辑游玩时间':`记录时间 · ${game.name}`;$('#timelineRecordType').value=type;$('#timelineRecordDate').value=event?.occurredAt||localDate();$('#timelineRecordPrecision').value=event?.datePrecision||'day';overlay.classList.add('open')}
      function closeTimelineRecordDialog(){$('#timelineRecordOverlay')?.classList.remove('open')}
      function saveTimelineRecord(event){const game=games().find(row=>String(row.id)===String(event.gameId));if(!game||!['started','completed'].includes(event.type))return;timelineUpsert({...event,title:timelineTypeLabel(event.type),note:'',source:event.source||'manual'});if(event.type==='completed')saveGames(games().map(row=>row.id===game.id?{...row,progress:100}:row))}
      $('#timelineRecordForm')?.addEventListener('submit',event=>{event.preventDefault();const type=$('#timelineRecordType').value,precision=$('#timelineRecordPrecision').value,gameId=$('#timelineRecordGameId').value,existingId=$('#timelineRecordEventId').value,row={id:existingId||backfillEventId(gameId,type),_replaceId:existingId,gameId,type,occurredAt:eventDateForPrecision($('#timelineRecordDate').value,precision),datePrecision:precision,source:'manual'};saveTimelineRecord(row);closeTimelineRecordDialog();renderTimeline();if($('#gameDetailPanel')&&!$('#gameDetailPanel').hidden)renderGameDetail(gameId);toast('游玩时间已保存')});
      ...
      function backfillEventId(gameId,type){return `backfill-${gameId}-${type}-main`}
